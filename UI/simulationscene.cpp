...
void SimulationScene::advance()
{
    m_grid->clear();

    // Step 1: Insert all vehicles
    for (Vehicle *v : m_Vehicles)
    {
        m_grid->insert(v);
    }

    // Step 2: Let each vehicle decide action
    for (Vehicle *v : m_Vehicles)
    {
        QList<Vehicle *> nearby = m_grid->getNearby(v, 50.0); // adjust radius
        Vehicle *leader = nullptr;

        // Simple nearest-leader search
        double minDist = 1e9;
        for (Vehicle *n : nearby)
        {
            double d = QLineF(v->pos(), n->pos()).length();
            if (n->get_position().x() > v->get_position().x() && d < minDist)
            {
                leader = n;
                minDist = d;
            }
        ...
        v->updateWithLeader(leader); // Replace per-vehicle collision logic
    }
}
